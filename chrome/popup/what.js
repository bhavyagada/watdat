document.addEventListener('DOMContentLoaded', () => {
  ...
  const updatePopupContent = async () => {
    ...
    const { highlighted, explained, apikey } = await chrome.storage.local.get(['highlighted', 'explained', 'apikey']);
  ...
  saveApiKeyButton.addEventListener('click', async () => {
    ...
    if (apiKey) {
      try {
        ...
        chrome.runtime.sendMessage({ action: 'api_key_updated', apikey: apiKey });
        errorContent.textContent = 'Highlight some text and press Ctrl+E to explain it.';
      } catch (error) {
  ...
  copyButton.addEventListener('click', async () => {
    const { explained } = await chrome.storage.local.get('explained');
    if (explained) {
      try {
  ...
  clearButton.addEventListener('click', async () => {
    try {
      await chrome.storage.local.remove(["highlighted", "explained"]);
      updatePopupContent();
    } catch (error) {
  ...
  chrome.runtime.onMessage.addListener((request) => {
    ...
    if (request.action === 'show_error') {
      errorContent.textContent = request.error || 'An error occurred.';
      errorContent.classList.remove('hidden');
      popupContent.classList.add('hidden');
    } else if (request.action === 'update_popup') {
      updatePopupContent();
    } else if (request.action === 'updated_explanation') {
      if (explainedText.textContent === 'Generating explanation...') {
        explainedText.textContent = request.partial_explanation;
      } else {
        explainedText.textContent += request.partial_explanation;
      }
    } else if (request.action === 'explanation_completed') {
